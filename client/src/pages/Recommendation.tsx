      ...
      <div className="max-w-4xl mx-auto md:px-10 pb-32">
        ...
        <div className="flex flex-col mt-4 md:mt-12 mb-6 md:mb-10 px-4 md:px-0">
          <div className="flex items-center gap-3 mb-1">
            <div className="p-2 bg-orange-600 rounded-xl shadow-lg shadow-orange-600/20 text-white">
              <Flame size={20} fill="currentColor" />
            </div>
            <h2 className="text-xl md:text-3xl font-black text-slate-800 tracking-tighter uppercase">Daily Picks</h2>
          </div>
        ...
        <div className="grid gap-3 md:gap-6 px-4 md:px-0">
          {RECOMMEND_DATA.map((item) => (
            <div 
              ...
              className="group relative flex items-center justify-between bg-white p-4 md:p-8 rounded-2xl md:rounded-[32px] border border-slate-300 shadow-sm hover:shadow-xl transition-all cursor-pointer active:scale-[0.98] hover:bg-slate-100/50"
            >
              <div className="flex items-center gap-4 md:gap-10 flex-1 min-w-0">
                {/* Rank Number */}
                <span className="text-2xl md:text-5xl font-black italic text-slate-100 group-hover:text-indigo-500/10 transition-colors leading-none tracking-tighter shrink-0">
                {item.rank}
                </span>
                ...
                <div className="flex flex-col min-w-0 gap-0.5">
                  <div className="flex items-center gap-2 flex-wrap">
                    <h3 className="text-[15px] md:text-2xl font-black text-slate-800 group-hover:text-indigo-600 transition-colors truncate tracking-tight">{item.name}</h3>
                    <div className="flex gap-1">
                      {item.tags.slice(0, 1).map(tag => (
                        <span key={tag} className="text-[8px] md:text-[10px] px-1.5 py-0.5 bg-slate-50 text-slate-400 rounded-md border border-slate-300 font-black uppercase">#{tag}</span>
                      ))}
                    </div>
                  </div>
                  ...
                  <div className="flex items-baseline gap-2.5 mt-1.5 md:hidden">
                    <span className="text-base font-black text-slate-800 font-mono tracking-tighter">{item.price.toLocaleString()}</span>
              ...
              {/* Desktop Price */}
              <div className="hidden md:flex flex-col items-end text-right px-10 shrink-0 border-r border-slate-300 mr-6">
                <div className="text-2xl font-black text-slate-800 font-mono tracking-tighter">{item.price.toLocaleString()}원</div>
              ...
              {/* Arrow */}
              <div className="w-10 h-10 md:w-12 md:h-12 rounded-xl md:rounded-2xl bg-slate-50 flex items-center justify-center text-slate-300 group-hover:bg-indigo-600 group-hover:text-white transition-all shadow-inner shrink-0">
        ...
        {/* Disclaimer */}
        <div className="mt-10 mx-2 md:mx-0 p-6 md:p-8 bg-white/50 rounded-[24px] md:rounded-[32px] border border-slate-300 border-dashed">
