...
const StockDetail = ({ favoritedStocks, onFavoriteToggle }: StockDetailProps) => {
  ...
  if (loading && !data) {
    return (
      <div className="flex flex-col h-screen bg-[#f8fafc] items-center justify-center">
        <RefreshCw className="animate-spin text-indigo-600 w-8 h-8 mb-4" />
  ...
  if (!data) {
    return (
      <div className="flex flex-col h-screen bg-[#f8fafc] items-center justify-center p-6 text-center">
        <p className="text-slate-500 font-bold mb-6 text-lg text-pretty">종목 정보를 찾을 수 없습니다.</p>
        <button onClick={() => navigate(-1)} className="px-10 py-4 bg-indigo-600 rounded-2xl font-black text-white shadow-lg shadow-indigo-600/20 active:scale-95 transition-all uppercase tracking-widest text-sm">뒤로가기</button>
      </div>
    );
  }
  ...
  return (
    <div className="flex flex-col h-screen bg-[#f8fafc] text-slate-800 overflow-hidden">
      {/* 상단 헤더 */}
      <header className="flex items-center p-4 border-b border-slate-300/60 bg-slate-50/90 backdrop-blur-md sticky top-0 z-10 shadow-sm">
        <button onClick={() => navigate(-1)} className="p-2 hover:bg-slate-50 rounded-full transition-colors mr-2">
          <ChevronLeft size={24} className="text-slate-600" />
        </button>
        <div className="flex-1">
          <div className="flex items-center gap-3">
            <h1 className="text-lg sm:text-2xl font-black text-slate-800 tracking-tight">{data.name}</h1>
            <button 
              onClick={() => onFavoriteToggle(data.code)}
              className={`p-2 rounded-xl transition-all border ${isFavorited ? 'bg-rose-50 border-rose-100 text-rose-500 shadow-sm' : 'bg-white border-slate-300 text-slate-300 hover:text-rose-500 hover:border-rose-100'}`}
            >
              <Heart size={18} fill={isFavorited ? "currentColor" : "none"} />
            </button>
            <span className="text-[11px] font-black text-slate-400 bg-slate-50 px-2 py-0.5 rounded border border-slate-300">{data.code} · {data.market}</span>
          </div>
          <div className="flex items-center gap-2 mt-1">
            <span className="text-xl sm:text-2xl font-mono font-black text-slate-800 leading-none">{data.price.toLocaleString()}</span>
            <span className={`text-sm sm:text-base font-black ${colorClass} bg-slate-50 px-2 py-0.5 rounded-lg border border-slate-300`}>
      ...
      {/* 탭 메뉴 */}
      <nav className="flex px-2 border-b border-slate-300/60 bg-slate-50/90 overflow-x-auto no-scrollbar whitespace-nowrap sticky top-[73px] z-10 backdrop-blur-md">
        {tabs.map((tab) => (
          <button
            key={tab}
      ...
      {/* 탭별 콘텐츠 영역 */}
      <main className="flex-1 overflow-y-auto p-4 md:p-8 pb-44 md:pb-12 bg-[#f8fafc]">
...
const ChartSection = ({ data, symbol }: { data: StockDetailData, symbol: string }) => {
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-500">
      <div className="bg-white rounded-[32px] overflow-hidden border border-slate-300 shadow-sm">
        <div className="p-5 border-b border-slate-300 flex justify-between items-center bg-slate-50/50">
          <div className="flex gap-1 bg-slate-200/50 p-1 rounded-2xl border border-slate-300/50">
            {timeframes.map((tf) => (
              <button
                key={tf.value}
                ...
                className={`px-5 py-2 text-xs font-black rounded-xl transition-all duration-300 ${
                  timeframe === tf.value 
                  ? 'bg-white text-indigo-600 shadow-md border border-slate-300' 
              ...
              <Tooltip
                ...
                content={({ active, payload, label }) => {
                  if (active && payload && payload.length) {
                    ...
                    const isUp = data.change >= 0;
                    return (
                      <div className="bg-white/95 backdrop-blur-md border border-slate-300 p-4 rounded-2xl shadow-2xl ring-1 ring-black/5">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        {[
          { label: '시가', value: data.open.toLocaleString(), color: 'text-slate-800' },
          { label: '거래량', value: data.volume.toLocaleString(), color: 'text-slate-800' },
          { label: '고가', value: data.high.toLocaleString(), color: 'text-rose-500' },
          { label: '저가', value: data.low.toLocaleString(), color: 'text-blue-600' }
        ].map((item, i) => (
          <div key={i} className="py-4 px-5 bg-white rounded-[24px] border border-slate-300 shadow-sm text-center transition-all hover:shadow-md">
...
const PriceSection = ({ data, symbol }: { data: StockDetailData, symbol: string }) => {
  ...
  return (
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-white rounded-[32px] overflow-hidden border border-slate-300 shadow-sm">
          <div className="p-5 border-b border-slate-100 bg-slate-50/50 flex justify-between items-center">
            <span className="text-[11px] font-black text-slate-600 uppercase tracking-[0.2em]">Live Order Book</span>
          ...
          {!hogaData || (hogaData.hoga && hogaData.hoga.length === 0) ? (
            ...
            <div className="flex flex-col">
              {hogaData.hoga.map((h: any, i: number) => (
                <div key={i} className={`flex items-center h-9 border-b border-slate-300 relative group ${h.sellQty > 0 ? 'bg-blue-50/20' : 'bg-rose-50/20'}`}>
                  ...
                  <div className={`flex-1 text-center font-mono font-black h-full flex items-center justify-center border-x border-slate-300 text-[13px] ${h.sellQty > 0 ? 'text-blue-600' : 'text-rose-500'}`}>
              ...
              <div className="flex border-t border-slate-300 bg-slate-50/50 text-[11px] font-black">
                <div className="flex-1 p-4 text-center border-r border-slate-300 text-blue-600 uppercase tracking-tighter">Sell Tot {hogaData.sellTotalQty.toLocaleString()}</div>
        ...
        <div className="flex flex-col gap-6">
          <div className="bg-white p-6 sm:p-8 rounded-[32px] border border-slate-300 grid grid-cols-2 gap-y-5 gap-x-8 shadow-sm">
            <div className="col-span-2 border-b border-slate-300 pb-3 mb-1">
              <span className="text-[11px] font-black text-slate-600 uppercase tracking-[0.2em]">Market Summary</span>
            </div>
            {[
              { label: '시가', value: data.open.toLocaleString() },
              { label: '전일종가', value: (data.price - data.change).toLocaleString(), muted: true },
              { label: '고가', value: data.high.toLocaleString(), color: 'text-rose-500' },
              { label: '상한가', value: (data.price * 1.3).toLocaleString().split('.')[0], color: 'text-rose-500' },
              { label: '저가', value: data.low.toLocaleString(), color: 'text-blue-600' },
              ...
              <div key={i} className="flex justify-between items-center border-b border-slate-300 pb-2.5">
                <span className="text-slate-400 text-[11px] font-black">{item.label}</span>
                <span className={`font-mono font-black text-sm ${item.color || (item.muted ? 'text-slate-400' : 'text-slate-800')}`}>{item.value}</span>
              </div>
            ))}
          </div>

          <div className="bg-white px-6 py-5 rounded-[24px] border border-slate-300 shadow-sm border-l-4 border-l-indigo-600">
      ...
      {/* 하단: 일자별 시세 */}
      <div className="bg-white rounded-[32px] overflow-hidden border border-slate-300 shadow-sm">
        <div className="p-5 border-b border-slate-300 bg-slate-50/50">
          <span className="text-[11px] font-black text-slate-600 uppercase tracking-[0.2em]">Daily Price History</span>
        </div>
        <div className="overflow-x-auto">
          <table className="w-full text-left text-xs">
            <thead className="bg-slate-50 text-slate-400 border-b border-slate-300 uppercase font-black tracking-tighter">
              <tr>
            ...
            <tbody className="divide-y divide-slate-300">
              {history.map((h, i) => {
                ...
                return (
                  <tr key={i} className="hover:bg-slate-100/50 transition-colors cursor-default">
                    <td className="py-4 px-6 text-slate-500 font-mono font-black">{h.date.replace(/(\d{4})(\d{2})(\d{2})/, '$2/$3')}</td>
                    <td className="py-4 px-6 text-right font-mono font-black text-slate-800 text-sm">{h.close.toLocaleString()}</td>
...
const InvestorSection = ({ symbol }: { symbol: string }) => {
  ...
  return (
    <div className="bg-white rounded-[32px] overflow-hidden border border-slate-300 shadow-sm animate-in fade-in slide-in-from-bottom-4 duration-500">
      <div className="p-5 border-b border-slate-300 bg-slate-50/50 flex justify-between items-center">
        <span className="text-[11px] font-black text-slate-600 uppercase tracking-[0.2em]">Investor Trends</span>
...
const InfoSection = ({ data }: { data: StockDetailData }) => (
  <div className="bg-white p-8 rounded-[32px] border border-slate-300 shadow-sm space-y-6">
    <h3 className="text-[11px] font-black text-slate-600 uppercase tracking-[0.2em] mb-6">Fundamental Data</h3>
    {[
      ...
      <div key={i} className="flex justify-between items-center border-b border-slate-300 pb-3 last:border-0 last:pb-0">
...
const NewsSection = () => (
  <div className="bg-white rounded-[32px] p-20 border border-slate-300 shadow-sm text-center">
...
const TradeSection = ({ data }: { data: StockDetailData }) => (
  <div className="bg-white rounded-[32px] overflow-hidden border border-slate-300 shadow-sm">
    <div className="p-5 border-b border-slate-300 bg-slate-50/50">
      <span className="text-[11px] font-black text-slate-600 uppercase tracking-[0.2em]">Recent 5 Days Flow</span>
    </div>
    <div className="overflow-x-auto">
        <table className="w-full text-left">
        <thead className="bg-slate-50 text-slate-400 font-black text-[10px] border-b border-slate-300 uppercase tracking-tighter">
            <tr>
            <th className="py-4 px-6">DATE</th>
            <th className="py-4 px-6 text-right">FOREIGN</th>
            <th className="py-4 px-6 text-right">INSTITUTION</th>
...
const CommunitySection = () => {
  ...
  return (
    <div className="space-y-6">
      <div className="bg-white p-6 rounded-[32px] border border-slate-300 shadow-sm group focus-within:ring-4 focus-within:ring-indigo-600/5 transition-all">
        <textarea
          value={comment}
          onChange={(e) => setComment(e.target.value)}
          placeholder="주주들과 자유롭게 의견을 나누어 보세요..."
          className="w-full bg-slate-50 border border-slate-300 rounded-2xl p-4 outline-none resize-none h-28 text-sm font-medium focus:bg-white transition-all"
        />
        <div className="flex justify-end mt-4">
          <button className="px-8 py-3 bg-indigo-600 rounded-2xl font-black text-sm text-white shadow-lg shadow-indigo-600/20 hover:bg-indigo-700 active:scale-95 transition-all uppercase tracking-widest">
            등록하기
          </button>
        </div>
      </div>
      <div className="bg-white p-6 rounded-[32px] border border-slate-300 shadow-sm border-l-4 border-l-indigo-600">
