...
import { Newspaper, Bell, Flame, Globe, Zap, Clock, ChevronRight } from 'lucide-react';
      ...
      <div className="max-w-4xl mx-auto px-4 md:px-10 pb-32">
        ...
        <div className="flex items-center justify-between mt-6 md:mt-12 mb-8 px-1">
          <div className="flex flex-col">
            <div className="flex items-center gap-3 mb-1">
                ...
                <h2 className="text-xl md:text-3xl font-black text-slate-900 tracking-tighter uppercase">Market Feed</h2>
            </div>
            <p className="text-slate-400 font-bold text-xs md:text-sm ml-1">시장을 움직이는 실시간 핵심 이슈</p>
          </div>
          <button className="p-2.5 bg-white border border-slate-200 rounded-xl text-slate-400 hover:text-indigo-600 shadow-sm active:scale-90 transition-all">
            <Bell size={20} />
          </button>
        </div>
        ...
        <div className="flex gap-1.5 mb-6 overflow-x-auto no-scrollbar px-1 md:px-0">
            {['전체', '증시', '산업', '경제', '해외', '속보'].map((cat, i) => (
                <button key={i} className={`px-5 py-2 rounded-full text-[11px] md:text-[13px] font-black whitespace-nowrap transition-all border-2 ${i === 0 ? 'bg-slate-900 border-slate-900 text-white shadow-lg' : 'bg-white border-slate-200 text-slate-400 hover:border-slate-300'}`}>
                    {cat}
                </button>
            ))}
        </div>
        ...
        <div className="bg-white rounded-2xl md:rounded-[32px] border border-slate-200 shadow-sm overflow-hidden">
          <div className="divide-y divide-slate-50">
            {NEWS_DATA.map((news) => (
              <div 
                key={news.id} 
                className="group flex items-center gap-3 md:gap-4 px-4 md:px-6 h-14 md:h-16 hover:bg-slate-50 transition-all cursor-pointer active:bg-blue-50/30"
              >
                {/* Hot Badge */}
                <div className="w-8 shrink-0 flex justify-center">
                    {news.isHot ? (
                        <div className="bg-rose-500 text-white text-[8px] font-black px-1.5 py-0.5 rounded uppercase animate-pulse">Hot</div>
                    ) : (
        ...
        {/* Footer Info */}
        <div className="mt-12 px-4 py-12 text-center border-t border-slate-200/60 md:mx-0">
