## 📝 2026년 2월 27일 업데이트: 종목 상세페이지 복구, 검색 시스템 구축 및 전 메뉴 모바일 최적화 (UX 고도화)

오늘은 서비스의 완성도를 결정짓는 **상세페이지의 데이터 정밀도 향상**과 **전체 종목 검색 기능**, 그리고 **모바일 사용자 경험(UX) 극대화**를 위한 고밀도 디자인 리팩토링 및 시스템 안정화를 완료했습니다.

### 1. 모바일 전용 커스텀 드롭다운 시스템 도입 (테마 선택)
- **네이티브 차단**: 모바일 브라우저에서 테마 선택 시 화면 전체를 덮어버리던 기본 `<select>` UI를 제거했습니다.
- **커스텀 UI 구현**: React `useState`를 활용한 독자적인 드롭다운 시스템을 구축하여, 화면 일부분만 차지하면서 부드럽게 목록을 보여주는 세련된 UX를 제공합니다.
- **안정성 확보**: 직접적인 DOM 조작 대신 React 상태 기반 렌더링으로 전환하여 런타임 오류와 화면 멈춤 현상을 근본적으로 해결했습니다.

### 2. Vercel SPA 라우팅 및 새로고침(404) 해결
- **Rewrites 정교화**: `vercel.json` 설정을 수정하여 `/explore`, `/news` 등 React Router의 가상 경로에서 사용자가 새로고침(아래로 당기기)을 할 때 발생하던 404 에러를 해결했습니다.
- **SPA 폴백 강화**: API 및 정적 자산 요청을 제외한 모든 경로를 `index.html`로 안전하게 연결하여 끊김 없는 브라우징 환경을 구축했습니다.

### 3. 종목 상세페이지 데이터 엔진 복구 및 고도화
- **실시간 호가(Hoga) 복구**: 네이버 Polling API 파싱 로직을 강화하여 '로딩 중' 현상을 해결하고, 매도/매수 잔량과 실시간 가격을 안정적으로 호출합니다.
- **매매동향 크롤링 전환**: 404 에러가 잦은 모바일 API 대신 **네이버 PC 버전 데이터 스크레이핑 방식**을 도입하여 외국인/기관 순매수 및 외인비율 데이터를 정확하게 확보했습니다.
- **시세 레이아웃 개편**: '시세' 탭을 **좌측(호가창) + 우측(주요 시세 정보)**의 2열 그리드로 재배치하고, 하단에 **일자별 시세 테이블**을 추가하여 정보의 깊이를 더했습니다.

### 4. 전체 시장 종목 검색 시스템 구축
- **검색 API 구현**: `krx_stocks.json` 내 4,224개 전체 상장 종목 데이터를 백엔드에 로드하여, 종목명/코드로 즉시 조회가 가능한 `/api/stocks/search` 엔드포인트를 구축했습니다.
- **탐색(Explore) UX 개선**: 탐색 메뉴에서 실시간 검색 바를 통해 종목을 찾고, 클릭 시 즉시 상세페이지로 이동하는 동선을 완성했습니다. 검색 결과 레이어는 독립 스크롤을 적용해 편의성을 높였습니다.

### 5. 전 메뉴 모바일 최적화 (Ultra-Dense Design)
- **고밀도 탭 내비게이션**: 상세페이지 탭 메뉴를 모바일에서 **2줄 그리드(4열)**로 변경하여 스크롤 없이 모든 메뉴를 즉시 선택할 수 있게 했습니다.
- **타이포그래피 정규화**: PC와 모바일의 폰트 크기가 제각각이던 문제를 해결하기 위해 환경별 표준 크기(`text-xs`, `text-sm` 등)를 적용하여 일관성을 확보했습니다.
- **하단 가림 방지 시스템**: 모바일 내비게이션 바에 콘텐츠 최하단이 가려지던 고질적 문제를 **반응형 패딩(`pb-44`) 및 안전 스페이서** 도입으로 완벽히 해결했습니다.

### 6. 안정성 및 배포 환경 개선
- **빌드 오류 해결**: TypeScript 컴파일 과정에서 미사용 임포트(`TrendingUp`, `ChevronRight` 등)로 인해 배포가 중단되던 에러를 수정하여 Vercel 배포 안정성을 확보했습니다.
- **방어적 데이터 렌더링**: 호가 데이터가 장 마감 등으로 비어있을 경우 크래시를 방지하고 "장 마감 안내"를 출력하는 로직을 보강했습니다.

---

## 📝 2026년 2월 25일 업데이트: 데이터 수집 아키텍처 최종 최적화 (네이버 + 토스 하이브리드)

오늘 작업은 한국투자증권 API의 법인 계약 제약을 완전히 회피하고, 서비스의 감성과 성능을 동시에 잡기 위해 **데이터 수집 체계를 하이브리드 방식(네이버 실시간 + 토스 정밀 크롤링)**으로 최종 확정 및 구현했습니다.

### 1. KIS API 의존성 완전 제거 및 보안 강화
- **토큰 발행 중단**: 법인 계약 이슈를 방지하기 위해 백엔드(`api/lib/kisApi.cjs`) 및 로컬 토큰 서버(`api/localTokenServer.mjs`) 내의 KIS API 호출 로직을 모두 차단했습니다.
- **안전한 데이터 전환**: 증권사 API 없이도 공신력 있는 데이터를 제공할 수 있도록 크롤링 기반으로 전면 교체했습니다.

### 2. 하이브리드 데이터 운영 전략 (Speed + Emotion)
- **네이버 금융 (Speed 담당)**: 
    - **실시간 지수(Ticker)**: 코스피, 코스닥 및 해외 지수를 1분 주기로 정밀 수집.
    - **시장 랭킹**: 급상승, 급하락, 거래량 상위 종목을 초고속으로 갱신.
    - **테마 랭킹**: 실시간 등락률이 포함된 테마 데이터를 네이버에서 직접 수집하여 정확한 수치를 보장.
- **토스증권 (Emotion 담당)**:
    - **투자자별 수급 동향**: 토스 특유의 세련된 UI에 담긴 개인/외국인/기관 순매수 데이터를 **Selenium 기반 파이썬 크롤러(`api/toss_investor_trend.py`)**로 수집.
    - **섹션별 개별 시간 매핑**: 각 투자자 주체별로 상이한 '기준 시간'(예: "오늘 10:54 기준")을 개별적으로 추출하여 화면에 표시.

### 3. 데이터 정밀도 및 UI 일관성 확보
- **95개 종목 고정**: 투자자별 리스트의 개수가 미세하게 차이 나는 문제를 해결하기 위해, 모든 섹션(외국인/기관/개인)의 수집 개수를 **정확히 95개**로 제한하여 UI 통일감을 확보했습니다.
- **단위 변환 엔진 최적화**: 토스에서 사용하는 **"조원"**, "억", "만" 등의 한글 단위 텍스트를 프론트엔드가 정확히 인식하여 표시하도록 로직을 보완했습니다.
- **정밀 스크롤 크롤링**: 토스 웹의 지연 로딩(Lazy Loading)을 극복하기 위해 크롤러에 **다단계 정밀 스크롤** 로직을 도입, 데이터 누락을 최소화했습니다.

### 4. 백엔드 및 DB 동기화 시스템 고도화
- **통합 캐시 아키텍처**: `api/update-data.js`가 네이버의 실시간 데이터와 파이썬이 긁어온 토스 JSON 데이터를 통합하여 Supabase DB에 원자적으로 업데이트합니다.
- **테마 상세 로딩 복구**: 테마를 네이버로 복구함에 따라, 특정 테마 클릭 시 해당 테마의 종목 리스트를 실시간으로 긁어오는 엔드포인트를 최적화했습니다.

### 5. 프론트엔드 안전성(Runtime Safety) 강화
- **런타임 에러 방지**: 데이터가 아직 수집되지 않았거나 비정상적인 값(null/undefined)이 들어올 경우 앱이 크래시(검정 화면)되는 현상을 방지하기 위해 전 섹션에 **방어적 렌더링(Defensive Rendering)** 코드를 적용했습니다.

### 6. 스마트 크론(Smart Cron) 자동화 및 자원 최적화
- **1분 단위 정밀 가동**: `vercel.json`의 스케줄을 `* * * * *`(1분)로 변경하여 실시간 대응력을 극대화했습니다.
- **시장 시간 지능형 감지 (KST 기준)**:
    - **한국 장 가동 (08:50 ~ 16:00)**: 장 오픈 전 예열부터 마감 직후까지 모든 데이터(네이버 1분, 토스 5분 주기)를 풀가동 수집합니다.
    - **미국 장 가동 (22:30 ~ 05:30)**: 상단 티커의 나스닥, S&P500 지수 변동을 실시간으로 반영하기 위해 글로벌 지수만 1분마다 업데이트합니다.
    - **슬립 모드 (심야/주말)**: 시장 휴장 시간에는 무거운 크롤링을 중단하여 Vercel 컴퓨팅 자원을 아끼고 보안 차단 위험을 원천 차단했습니다.
- **이원화 자동 수집 엔진**:
    - **네이버**: 매 분마다 지수 및 랭킹을 즉시 수집하여 진짜 실시간 서비스를 실현했습니다.
    - **토스**: 5분마다 Node.js 환경에서 브라우저를 자동 실행하여 정밀한 수급 데이터를 안전하게 낚아챕니다.

### 7. 데이터 증발 문제 해결 및 DB 철벽 방어 시스템 (The Final Lock)
- **문제 분석**: 1분 주기(네이버)와 5분 주기(토스) 업데이트가 교차하는 과정에서, 특정 행이 사라지거나 빈 값으로 덮어씌워지는 현상 발생.
- **근본 원인**:
    - **환경 충돌**: Vercel 서버리스 환경에서 토스 크롤링 실패 시 `null` 데이터를 DB에 `upsert` 하려 시도함.
    - **DB 구조 결함**: 기존 테이블의 `id` 컬럼에 기본키(PK) 제약이 없어 데이터가 중복되거나 `.single()` 호출 시 에러 발생.
- **최종 해결책 (이중 잠금)**:
    - **백엔드 로직 격리**: `update-data.js`에서 Vercel 서버 환경일 경우 토스 행(`toss_investor_trend_all`)에 대한 접근 및 수정을 원천적으로 차단하도록 분리 구현.
    - **DB 레벨 보안 (SQL)**: 
        - `REVOKE DELETE`: 모든 사용자로부터 삭제 권한을 박탈하여 데이터 증발 방지.
        - `RLS Policy`: 특정 핵심 행에 대해 '업데이트 잠금' 정책을 적용하여 허가되지 않은 서버의 쓰기 시도를 DB 차원에서 거부.
        - `PK 강제 지정`: 중복 데이터 발생을 막기 위해 `id`를 기본키로 고정.
- **성과**: 네이버 지수는 1분마다 생생하게 업데이트되면서도, 정밀한 토스 수급 데이터는 삭제나 오염 걱정 없이 영구적으로 보존되는 '철벽 아키텍처' 완성.

---

## 📝 2026년 2월 20일 업데이트: 브랜드 리브랜딩(StockMate) 및 전체 데이터 자동화 시스템 구축

성공적인 배포 이후, 서비스의 브랜드 정체성을 확립하고 데이터 범위를 테스트 수준에서 정식 서비스 수준으로 전면 확대했습니다.

### 1. 브랜드 리브랜딩 및 SEO 최적화
- **브랜드명 확정**: 기존 `StockLive`에서 **`StockMate(스탁메이트)`**로 서비스명 변경. 투자 동반자라는 브랜드 가치 강조.
- **도메인 연결**: 정식 도메인을 **`mystockmate.com`**으로 결정하고 관련 설정 준비.
- **SEO 강화**: `index.html` 내 메타 태그(Description, Keywords) 및 Open Graph 설정을 업데이트하여 검색 엔진 노출 최적화.
- **UI 반영**: 헤더 로고 및 사이트 타이틀을 `STOCKMATE`로 전면 교체.

### 2. 데이터 범위 전면 확대 (Full-Scale Data)
- **테마 및 종목 확장**: 기존 테스트용 150개 테마에서 수집된 모든 테마(`toss_stock_themes_local_v3.json`)와 전 종목(`krx_stocks.json`) 데이터를 사용하도록 백엔드 로직 수정.
- **데이터 로딩 최적화**: 수천 개의 종목 정보를 효율적으로 매핑하기 위해 `Map` 오브젝트를 도입하여 데이터 처리 속도 개선.

### 3. 하이브리드 캐시 업데이트 시스템 구축
데이터의 최신성을 유지하면서 서버 부하를 최소화하기 위해 두 가지 방식의 업데이트 스크립트 운영:
- **수동 업데이트 (`api/forceUpdate.js`)**: 
    - 개발자가 로컬 터미널에서 `node forceUpdate.js`를 실행하여 즉시 DB 캐시를 갱신. 
    - 초기 배포 직후나 대규모 데이터 변경 시 사용.
- **자동 업데이트 (`api/update-data.js` + Vercel Cron)**: 
    - Vercel Pro 요금제를 활용하여 **5분 주기**로 자동 실행.
    - KIS API 호출 제한을 고려하여 300초(5분)의 넉넉한 `maxDuration` 설정.
    - 사용자는 별도의 계산 대기 없이 언제나 5분 이내의 최신 순위 데이터를 즉시 열람 가능.

### 4. 주요 버그 수정 및 API 보완
- **테마 상세 종목 미표시 해결**: `/api/themes/:themeName/stocks` 엔드포인트를 추가하여 사용자가 테마 클릭 시 해당 테마의 종목 리스트를 DB 캐시에서 즉시 불러오도록 수정.
- **JSON 파싱 오류 수정**: 신규 테마 파일(`v3`)의 배열 구조가 기존과 다른 점을 인지하여, 데이터 로딩 시 형식을 자동 판별하도록 로직 보완.

---
## 📝 2026년 2월 20일 업데이트: Vercel 배포 빌드 오류(PostCSS) 및 데이터 연동 문제 해결

오늘은 Vercel 배포 과정에서 발생한 심규한 빌드 오류와 데이터 연동 문제를 해결하여 성공적으로 서비스를 배포했습니다.

### 1. Vercel 빌드 오류 해결 (PostCSS Native Binding 이슈)

**문제 현상:**
- Vercel(Linux 환경)에서 빌드 중 `Failed to load PostCSS config` 및 `Cannot find native binding` 에러 발생.
- `lightningcss` 또는 `rollup`의 리눅스용 바이너리를 찾지 못해 빌드가 중단되는 현상.

**원인 분석:**
- **Tailwind CSS v4 아키텍처 변화**: Tailwind v4는 내부적으로 `lightningcss`를 사용하는데, 기존 PostCSS 방식(`postcss.config.cjs`)을 사용할 경우 배포 환경(Linux)과 개발 환경(Windows) 간의 네이티브 모듈 충돌이 잦음.
- **의존성 정보 누락**: `package-lock.json` 생성 시 개발 환경의 바이너리 정보만 포함되어 Vercel 리눅스 환경에서 필요한 모듈을 내려받지 못함.
- **불필요한 빌드 로직**: 루트 `package.json`에서 빌드 시마다 `node_modules`를 강제로 지우고 다시 설치하는 로직이 Vercel의 캐싱 시스템과 충돌하여 바이너리 설치를 방해함.

**해결 방법 (최종):**
1.  **PostCSS 방식 폐기**: `postcss.config.cjs` 파일을 삭제하고 PostCSS 의존성을 제거.
2.  **Vite 전용 플러그인 도입**: Tailwind CSS v4 전용 Vite 플러그인(`@tailwindcss/vite`)을 도입하여 Vite가 직접 CSS를 처리하도록 변경. 이 방식은 PostCSS 로딩 단계를 생략하므로 네이티브 바인딩 오류를 근본적으로 차단함.
3.  **멀티 플랫폼 아키텍처 설정**: `.npmrc`에 `supportedArchitectures.os=linux,win32`를 명시하여 `package-lock.json`에 두 환경의 정보를 모두 담음.
4.  **빌드 스크립트 단순화**: 루트 `package.json`의 빌드 명령어를 `cd client && npm run build`로 단순화하여 Vercel 표준 설치 프로세스를 따르도록 수정.

### 2. 배포 후 데이터 표시 오류 해결

**문제 현상:**
- 배포 성공 후에도 화면에 주식 데이터가 나타나지 않는 현상 발생.

**원인 및 해결:**
- **환경 변수 주입**: Vercel 프로젝트 설정에 KIS API 및 Supabase 관련 환경 변수(`VITE_...`)가 정확히 입력되었는지 확인.
- **백엔드/프론트엔드 연결**: Vercel 서버리스 함수(`api/*.js`)와 프론트엔드(`client/dist`) 간의 라우팅 설정(`vercel.json`)을 정밀하게 조정하여 `/api` 요청이 올바르게 백엔드 코드로 전달되도록 조치.
- **Node.js 버전 일치**: 루트와 클라이언트의 Node.js 버전을 22.x로 상향 조정하여 최신 라이브러리 호환성 확보.

### 3. 향후 유지보수 가이드
- **Tailwind 설정 변경 시**: `postcss.config.cjs`를 다시 만들지 마십시오. 모든 설정은 `vite.config.ts`와 `tailwind.config.js` 또는 CSS 내의 `@theme` 지시어를 통해 관리합니다.
- **배포 오류 발생 시**: `client/package-lock.json`을 삭제하고 로컬에서 `npm install`을 다시 실행하여 멀티 플랫폼 메타데이터가 포함된 lock 파일을 새로 생성한 뒤 푸시하십시오.

---

## 📝 2026년 2월 9일 업데이트: 홈페이지 성능 최적화 및 테마/종목 정보 표시 리팩토링

오늘은 StockMate 홈페이지의 심각한 성능 문제와 관련된 대규모 리팩토링 및 디버깅 작업을 수행했습니다. 목표는 과도한 API 호출과 클라이언트 측 과부하를 해결하여, 사용자에게 빠르고 안정적인 실시간 주식 정보를 제공하는 것이었습니다.

### 1. 문제 진단 및 아키텍처 변경

**초기 문제점:**
- **과도한 KIS API 호출**: 프론트엔드(`App.tsx`)에서 1000개가 넘는 모든 주식의 가격을 개별적으로 KIS API에 요청하여 네트워크 부하가 극심했습니다.
- **클라이언트 측 과부하**: `Home.tsx`에서 모든 테마의 평균 등락률을 계산하고 정렬하는 등, 브라우저에서 너무 많은 연산을 수행하여 성능 저하 및 앱 크래시를 유발했습니다.

**해결을 위한 아키텍처 변경:**
- **백엔드 중심 데이터 처리**: 모든 KIS API 호출과 복잡한 데이터 처리(등락률 계산, 정렬)를 Node.js 기반의 백엔드 서버로 이관하기로 결정했습니다.
- **프론트엔드의 단순화**: 프론트엔드는 이제 백엔드로부터 미리 가공된 데이터를 받아 화면에 표시하는 역할만 수행합니다.

### 2. 구현 및 디버깅 과정

**2.1. 백엔드 개발 (`stocklive/server`)**

-   **핵심 KIS API 연동**:
    -   `getKisToken` 함수를 구현하여 KIS API 인증 토큰을 발급받고 캐싱합니다.
    -   `fetchStockPrice` 함수를 구현하여 개별 주식의 가격 정보를 조회하고 캐싱합니다. 이 함수는 네트워크 오류(예: `socket hang up`)에 대비해 **재시도 메커니즘**과 **타임아웃(5초)**을 포함합니다.
    -   **Critical Fix**: `fetchStockPrice`에서 KIS API의 요구사항에 맞게 `tr_id`를 `FHKST01010100`으로 변경하고, 누락되었던 `Content-Type: application/json; charset=utf-8` 및 `custtype: 'P'` 헤더를 추가하여 KIS API가 주식 가격 요청을 거부하는 문제를 해결했습니다.
    -   **디버깅 강화**: `fetchStockPrice`에서 KIS API 응답 및 오류를 더 상세하게 로깅하도록 개선했습니다.
-   **새로운 API 엔드포인트 구현**:
    -   **`/api/themes/top-performing`**: 모든 테마의 주식 가격을 KIS API에서 가져와 테마별 평균 등락률을 계산하고, 상위 테마 목록을 정렬하여 반환합니다. KIS API 부하를 줄이기 위해 **10개씩 청크 단위로 병렬 요청**하고 청크 사이에 **500ms 지연**을 줍니다.
    -   **`/api/themes/:themeName/stocks`**: 특정 테마에 속한 종목 목록을 조회하여 실시간 가격 정보와 함께 정렬하여 반환합니다.
    -   **`/api/stocks/prices`**: 여러 종목 코드를 받아 해당 종목들의 가격 정보를 한 번에 반환하여 헤더 마퀴 및 관심 종목 목록에 사용됩니다.
-   **데이터 파싱 및 매핑 오류 수정**:
    -   `toss_real_150_themes.json` 파일이 객체 내 `themes` 배열을 포함하고 있어 `JSON.parse` 후 `parsedData.themes`로 접근하도록 수정했습니다.
    -   **Critical Fix**: `/api/themes/:themeName/stocks` 엔드포인트에서 테마 이름을 찾을 때 `t.name` 대신 `t.theme_name`을 사용하도록 수정하여 404 오류를 해결했습니다.
    -   `fetchStockPrice`에서 `tradeVolume` 및 `tradeValue`가 `null`/`undefined`일 경우 기본값 `0`으로 설정하도록 개선했습니다.
    -   **종목명 문제 디버깅**: KIS API에서 가져오는 `output.hts_korp_isnm`이 프론트엔드에서 제대로 표시되지 않는 문제를 디버깅하기 위해, `fetchStockPrice`에서 `name` 속성을 `String(output.hts_korp_isnm || stockCode).trim()`으로 할당하도록 수정했습니다.
        -   이전 단계에서는 `name: "TEST_NAME"`을 하드코딩하여 `name` 필드가 성공적으로 프론트엔드에 전달될 수 있음을 확인했습니다.
        -   현재 `output.hts_korp_isnm`을 `String()`으로 명시적으로 변환하고 `trim()`한 후, `stockCode`를 대체 값으로 사용하는 방식으로 최종 수정했습니다.

**2.2. 프론트엔드 리팩토링 (`stocklive/client`)**

-   **`App.tsx`**:
    -   KIS API와 직접 통신하던 모든 기존 로직(토큰 발급, 개별 주식 가격 조회 루프)을 제거했습니다.
    -   새로 만든 백엔드 `/api/stocks/prices` 엔드포인트를 호출하여 마퀴 및 관심 종목에 필요한 주식 가격을 주기적으로 가져오도록 수정했습니다.
    -   사용되지 않는 `delay` 유틸리티 함수를 제거했습니다.
    -   **Critical Fix**: 마퀴의 등락률 색상(빨강/파랑) 및 퍼센트 표시를 `stock.changeRate`를 기반으로 직접 계산하도록 수정했습니다.
-   **`Home.tsx`**:
    -   클라이언트 측에서 테마 데이터를 처리(가져오기, 계산, 정렬)하던 모든 로직을 제거했습니다.
    -   새로운 백엔드 엔드포인트(`/api/themes/top-performing`, `/api/themes/:themeName/stocks`)를 호출하여 데이터를 가져오도록 수정했습니다.
    -   API 호출 실패 시 앱이 크래시되지 않도록 견고한 **오류 처리 로직**을 추가했습니다.
    -   리스트 렌더링 시 `key` prop에 고유한 `stock.code`를 사용하도록 수정했습니다.
    -   `stock.change` 대신 `stock.changeRate`를 사용하여 등락률을 정확히 표시하도록 수정했습니다.
    -   `stock.changeRate`가 `undefined`일 경우 `toFixed(2)` 호출 시 크래시되지 않도록 `|| 0` 대체값을 추가했습니다.
-   **`vite.config.ts`**:
    -   오래된 KIS API 프록시 설정을 제거하고, 모든 `/api` 요청을 `http://localhost:4000`에서 실행 중인 새로운 Node.js 백엔드 서버로 리다이렉션하도록 프록시 설정을 업데이트했습니다.

## 3. 남아있는 문제 및 향후 계획

현재 가장 시급한 문제는 **종목명이 숫자로 표시되는 현상**입니다. (예: "삼성전자" 대신 "005930"으로 표시)
-   최근 적용된 수정 사항(`name: (typeof output.hts_korp_isnm === 'string' ? output.hts_korp_isnm : stockCode).trim()`)이 이 문제를 해결할 것으로 기대합니다.

**향후 계획:**

1.  **종목명 문제 최종 확인**: 현재 적용된 수정 사항이 종목명을 올바르게 표시하는지 확인합니다.
2.  **나머지 카테고리 구현**: 티커 및 테마를 제외한 `급상승`, `급하락`, `거래량`, `거래대금` 등의 시장 순위 카테리에 실제 데이터를 연결하는 작업을 진행합니다. 이를 위해 백엔드에 새로운 API 엔드포인트를 구현하고 프론트엔드를 업데이트할 예정입니다.
3.  **Vercel 배포 확인**: 모든 로컬 작업이 완료되면 Vercel 배포 환경에서도 문제가 없는지 확인합니다.

---

## 📝 2026년 2월 8일 업데이트: NPM 취약점 해결, 로컬 개발 환경 구성 및 Recharts 경고 해결

오늘은 NPM 의존성 취약점, 로컬 개발 환경 설정 문제, Vercel 배포 오류, 그리고 Recharts 라이브러리 경고 해결에 집중했습니다.

### 1. NPM 취약점 및 의존성 문제 해결
*   **문제:** 반복적인 `npm audit fix --force` 실행에도 불구하고 NPM 의존성 취약점 해결이 되지 않고, `@vercel/node` 버전이 계속 변경되는 "의존성 지옥" 현상이 발생했습니다. 이는 `npm audit`이 부정확한 패치 버전을 제시하고, `package.json`의 `type: module` 설정과 `__dirname` 사용 방식이 충돌하며, `^`를 포함한 버전 오버라이드 때문에 `ETARGET` 오류가 발생했기 때문입니다.
*   **해결:**
    *   `client/package.json` 파일에서 `@vercel/node` 의존성을 안정적인 버전인 `^5.5.28`로 되돌려 안정성을 확보했습니다.
    *   `client/package.json`의 `overrides` 섹션에 `esbuild@0.25.0`, `undici@6.23.0`, `path-to-regexp@8.3.0`, `tar@7.5.7` 등 문제가 된 트랜지티브 의존성들의 정확하고 패치된 버전을 명시적으로 추가했습니다.
    *   `overrides` 버전에서 `^` 기호를 제거하여 `ETARGET` 오류를 방지했습니다.
    *   **결과:** 모든 `npm audit` 취약점이 성공적으로 해결되었습니다.

### 2. 로컬 개발 환경 KIS API 환경 변수 로딩 및 Vercel 배포 문제 해결
*   **문제:** 로컬 개발 환경에서 앱 실행 시 화면이 하얗게 뜨고 `localTokenServer.js`에서 `KIS_APP_KEY: Not Set` 오류가 발생했습니다. 또한, Vercel 배포 시 `tsc: command not found` 오류로 빌드가 실패했습니다.
*   **분석:**
    *   로컬 환경 문제는 `client/server/localTokenServer.js` 파일이 ES 모듈로 처리되면서 `__dirname`이 정의되지 않아 `.env.local` 파일을 올바르게 로드하지 못했기 때문입니다.
    *   Vercel 배포 오류는 `typescript`가 `devDependencies`에 포함되어 빌드 시 `tsc`를 찾지 못했기 때문입니다.
*   **해결:**
    *   `client/server/localTokenServer.js` 파일에 `import.meta.url`을 사용하여 `__filename`과 `__dirname`을 ES 모듈 환경에서 올바르게 정의하는 코드를 추가했습니다.
    *   `dotenv.config` 호출 경로를 `path.resolve(__dirname, '..', '.env.local')`로 수정하여 `.env.local` 파일을 스크립트 위치 기준으로 정확하게 로드하도록 했습니다.
    *   Vercel 배포 설정에서 빌드 명령을 `npm install && npm run build`로 재정의하여 `devDependencies`를 포함한 모든 의존성이 설치되도록 했습니다.
    *   **결과:** 로컬 개발 서버에서 KIS 환경 변수가 성공적으로 로드되고 앱이 정상적으로 실행되었으며, Vercel 배포도 성공했습니다.

### 3. Recharts 차트 경고 및 레이아웃 조정
*   **문제:** `client/src/pages/Home.tsx` 파일의 차트 컴포넌트에서 `recharts` 경고(`The width(-1) and height(-1) of chart should be greater than 0`)가 지속적으로 발생했습니다. 이 경고를 해결하려는 초기 시도(고정 높이 `h-12` 또는 `style={{height: '24px'}}` 적용)는 차트의 크기를 변경하여 종목 리스트 행의 높이를 늘리는 부작용을 초래했습니다.
*   **분석:** `ResponsiveContainer`가 CSS Grid 레이아웃 내에서 부모 컨테이너로부터 유효한 치수(너비/높이)를 얻지 못하고 있었으며, 백분율 기반이나 `aspect` 비율을 사용하는 방식으로는 이 문제가 해결되지 않았습니다.
*   **해결:** `client/src/pages/Home.tsx` 파일에서 `ResponsiveContainer` 컴포넌트에 `width={108}` 및 `height={36}`과 같이 명시적인 픽셀 값을 지정했습니다. 이는 차트가 배치되는 그리드 컬럼의 너비(120px)를 고려하여 설정되었으며, 차트가 음수 치수를 보고하는 문제를 해결합니다.
*   **결과:** `recharts` 경고가 사라졌으며, 차트가 이전보다 약간 커졌지만 전반적인 레이아웃을 크게 해치지 않는 선에서 허용 가능한 수준이 되었습니다.

---
## 📝 2026년 1월 27일 업데이트: Git 충돌 해결 및 배포 오류 디버깅, KIS 토큰 캐싱 구현

오늘은 `git push` 과정에서 발생한 `non-fast-forward` 오류와 이어지는 배포 문제들을 해결하는 데 집중했습니다.

### 1. Git 병합 충돌 해결
*   **문제:** 원격 저장소와의 불일치로 인해 `git push`가 거부되었고, `git pull` 시 `client/package.json`, `client/src/App.tsx`, `client/vite.config.ts` 파일에서 병합 충돌이 발생했습니다.
*   **해결:** 수동으로 충돌 마커(<<<<<<< HEAD, =======, >>>>>>> branch_name)를 제거하고 코드를 통합하여 병합 충돌을 해결했습니다.

### 2. Vercel 배포 오류 디버깅 및 코드 수정
여러 배포 오류가 순차적으로 발생했으며, 각 문제에 대한 디버깅과 코드 수정이 이루어졌습니다.

*   **`client/vite.config.ts` 구문 오류 수정:**
    *   **문제:** 배포 과정에서 `vite.config.ts` 파일에 불필요한 중괄호(`}`)로 인한 `SyntaxError: Expected ")" but found "}"`가 발생했습니다.
    *   **해결:** `vite.config.ts` 파일 내의 객체 구조를 정확하게 수정하여 구문 오류를 해결했습니다.

*   **`req.headers.get is not a function` 오류 수정 (Vercel 환경):**
    *   **문제:** Vercel 환경에서 `client/api/uapi/[...path].ts` 파일 내 `req.headers.get()` 호출 시 `req.headers.get is not a function` 오류가 발생했습니다. Vercel의 서버리스 함수는 `req.headers`를 표준 `Headers` 객체가 아닌 일반 JavaScript 객체로 제공하기 때문이었습니다.
    *   **해결:** `req.headers` 객체에 대괄호 표기법(`req.headers['header-name']`)을 사용하여 헤더 값에 접근하도록 `client/api/uapi/[...path].ts` 파일을 수정했습니다. 또한, 타입스크립트 호환성을 위해 `as Record<string, string>` 타입 단언을 추가했습니다.

*   **Vercel KV 제거로 인한 `SyntaxError: Illegal return statement`:**
    *   **문제:** 사용자 요청에 따라 Vercel KV 캐싱 로직(`@vercel/kv` 의존성)을 코드에서 제거하는 과정에서 `client/api/uapi/[...path].ts` 파일의 JSON 파싱(`data = await response.json()`)을 감싸는 `try...catch` 블록이 손상되어 `Illegal return statement` 오류가 발생했습니다.
    *   **해결:** `try { data = await response.json(); } catch (...)` 구조를 올바르게 복원하여 구문 오류를 해결했습니다.

### 3. KIS API 토큰 발급 속도 제한 문제 해결 (인메모리 캐싱)
*   **문제:** 위 오류들을 해결한 후 배포 시 KIS API (`openapi.koreainvestment.com`)로부터 "접근토큰 발급 잠시 후 다시 시도하세요(1분당 1회)"라는 `403 Forbidden` (`EGW00133`) 응답이 발생하여 토큰 발급 속도 제한에 걸리는 문제가 확인되었습니다. 이전 Vercel KV 캐싱 로직이 제거되었기 때문에 발생한 문제였습니다.
*   **해결:** Vercel KV를 사용하지 않는 대안으로 `client/api/uapi/[...path].ts` 파일 내에 KIS 토큰과 그 만료 시간을 저장하는 **인메모리 캐싱 메커니즘**을 구현했습니다. 이는 전역 변수를 사용하여 서버리스 함수 인스턴스의 생애 주기 동안 토큰을 재활용함으로써 KIS API에 대한 반복적인 토큰 발급 요청을 줄여 속도 제한 문제를 완화합니다.

---
## ⚠️ 2026년 1월 27일 업데이트: 실시간 주가 티커 오류 해결 시도 및 현재 상황
이번 문제는 단순히 코드 한 줄의 실수가 아니라, 프론트엔드 빌드 도구(Vite)와 백언드 API 서버 사이의 통신 규칙이 어긋나서 발생한 복합적인 문제였습니다. 이해하기 쉽게 핵심만 요약해 드릴게요.

1. 문제의 원인 (Why?)
가장 큰 원인은 **"길 찾기 실패"**였습니다.

프록시(Proxy) 미작동: App.tsx에서 /uapi/...로 요청을 보냈지만, vite.config.ts 설정은 /api/uapi라는 신호가 올 때만 도와주기로 약속되어 있었습니다.

엉뚱한 응답 (HTML vs JSON): 요청을 어디로 보낼지 몰랐던 브라우저는 결국 자기 자신(index.html)을 가져왔습니다. 코드는 "주식 가격(JSON)"을 기다리고 있는데 "웹사이트 본문(HTML)"이 들어오니 < 문자를 보고 에러(Unexpected token <)를 뱉은 것이죠.

설정 파일 충돌: vite.config.js와 ts가 공존하면서 Vite가 어떤 규칙을 따라야 할지 혼란을 겪고 있었습니다.

2. 해결 방법 (How?)
우리는 세 단계를 통해 이 꼬인 매듭을 풀었습니다.

통신 경로 일원화:

App.tsx의 모든 요청 주소 앞에 /api를 붙여서 Vite가 **"아, 이건 증권사로 전달해줘야 하는 거구나!"**라고 확실히 인지하게 만들었습니다.

설정 파일 정제:

중복되던 .js 파일을 지우고 .ts 파일로 통합했습니다.

package.json에서 명령어를 수정해 Vite가 올바른 설정 파일을 읽도록 강제했습니다.

캐시 리셋 (--force):

브라우저와 서버가 기억하고 있던 "잘못된 길(구 버전 cache)"을 강제로 삭제하고 새 길을 찾게 했습니다.
