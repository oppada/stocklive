## 📝 2026년 2월 21일 업데이트: 데이터 수집 엔진 전면 개편 및 네이버 금융 크롤링 도입

오늘 작업은 한국투자증권 API의 법인 계약 제한 및 권한 문제를 극복하고, 서비스의 실시간성과 데이터 신뢰도를 확보하기 위해 **데이터 수집 엔진을 하이브리드 방식(KIS API + 네이버 금융 크롤링)**으로 전면 개편했습니다.

### 1. 네이버 금융 기반 실시간 지수 및 랭킹 수집
- **지수 수치 정상화**: KIS API 권한 문제로 인해 부정확하게 표시되던 지수(KOSPI, KOSDAQ 등)를 **네이버 금융 실시간 폴링 API**로 교체했습니다. 소수점 보정 로직을 통해 실제 지수 포인트를 정확하게 표시합니다.
- **크롤링 엔진 도입 (`cheerio`, `iconv-lite`)**: 네이버 금융의 HTML을 직접 파싱하는 엔진을 구축했습니다. 이를 통해 법인 계약 이슈 없이도 급상승, 급하락, 거래량, 거래대금 상위 50개 종목을 안정적으로 수집합니다.
- **EUC-KR 인코딩 해결**: 네이버 금융 특유의 한글 깨짐 문제를 `iconv-lite` 버퍼 디코딩으로 완벽하게 해결했습니다.

### 2. 시장 랭킹 수집 로직 고도화 및 수치 보정
- **컬럼 자동 매핑**: 네이버 금융의 각 시세 페이지(급상승 vs 거래량)마다 다른 테이블 구조를 자동으로 인식하여 **현재가, 등락률, 거래량, 거래대금**의 위치를 정확히 매핑하도록 로직을 강화했습니다.
- **단위 환산 및 정제**: 거래대금(백만 원 -> 원) 단위 환산 및 특수 문자(화살표, %, + 등) 제거 로직을 적용하여 데이터의 일관성을 확보했습니다.
- **실시간 테마 순위 연동**: 네이버 금융의 테마 시세 페이지를 추가로 크롤링하여, 메인 화면에서 실제 시장을 주도하는 실시간 테마 정보를 열람할 수 있게 되었습니다.

### 3. 하이브리드 데이터 운영 체계 확립
- **이원화 수집**: 
    - **네이버 금융**: 지표(지수, 환율), 시장 랭킹(Top 50), 테마 데이터 전담.
    - **KIS API**: 4,000여 개 전 종목의 기초 가격 수집 및 검색용 데이터 활용.
- **시스템 안정화**: 수집 간 지연 시간(Delay) 조정 및 상세 로그 시스템을 통해 `socket hang up` 및 `404` 에러를 최소화하고, 수집 실패 시 자동 폴백(Fallback) 로직을 적용했습니다.
- **토큰 관리 개선**: KIS 토큰을 Supabase DB에 캐싱하여 불필요한 인증 문자 발송을 차단하고 24시간 동안 안정적으로 토큰을 재사용합니다.

---
## 📝 2026년 2월 20일 업데이트: 브랜드 리브랜딩(StockMate) 및 전체 데이터 자동화 시스템 구축

성공적인 배포 이후, 서비스의 브랜드 정체성을 확립하고 데이터 범위를 테스트 수준에서 정식 서비스 수준으로 전면 확대했습니다.

### 1. 브랜드 리브랜딩 및 SEO 최적화
- **브랜드명 확정**: 기존 `StockLive`에서 **`StockMate(스탁메이트)`**로 서비스명 변경. 투자 동반자라는 브랜드 가치 강조.
- **도메인 연결**: 정식 도메인을 **`mystockmate.com`**으로 결정하고 관련 설정 준비.
- **SEO 강화**: `index.html` 내 메타 태그(Description, Keywords) 및 Open Graph 설정을 업데이트하여 검색 엔진 노출 최적화.
- **UI 반영**: 헤더 로고 및 사이트 타이틀을 `STOCKMATE`로 전면 교체.

### 2. 데이터 범위 전면 확대 (Full-Scale Data)
- **테마 및 종목 확장**: 기존 테스트용 150개 테마에서 수집된 모든 테마(`toss_stock_themes_local_v3.json`)와 전 종목(`krx_stocks.json`) 데이터를 사용하도록 백엔드 로직 수정.
- **데이터 로딩 최적화**: 수천 개의 종목 정보를 효율적으로 매핑하기 위해 `Map` 오브젝트를 도입하여 데이터 처리 속도 개선.

### 3. 하이브리드 캐시 업데이트 시스템 구축
데이터의 최신성을 유지하면서 서버 부하를 최소화하기 위해 두 가지 방식의 업데이트 스크립트 운영:
- **수동 업데이트 (`api/forceUpdate.js`)**: 
    - 개발자가 로컬 터미널에서 `node forceUpdate.js`를 실행하여 즉시 DB 캐시를 갱신. 
    - 초기 배포 직후나 대규모 데이터 변경 시 사용.
- **자동 업데이트 (`api/update-data.js` + Vercel Cron)**: 
    - Vercel Pro 요금제를 활용하여 **5분 주기**로 자동 실행.
    - KIS API 호출 제한을 고려하여 300초(5분)의 넉넉한 `maxDuration` 설정.
    - 사용자는 별도의 계산 대기 없이 언제나 5분 이내의 최신 순위 데이터를 즉시 열람 가능.

### 4. 주요 버그 수정 및 API 보완
- **테마 상세 종목 미표시 해결**: `/api/themes/:themeName/stocks` 엔드포인트를 추가하여 사용자가 테마 클릭 시 해당 테마의 종목 리스트를 DB 캐시에서 즉시 불러오도록 수정.
- **JSON 파싱 오류 수정**: 신규 테마 파일(`v3`)의 배열 구조가 기존과 다른 점을 인지하여, 데이터 로딩 시 형식을 자동 판별하도록 로직 보완.

---
## 📝 2026년 2월 20일 업데이트: Vercel 배포 빌드 오류(PostCSS) 및 데이터 연동 문제 해결

오늘은 Vercel 배포 과정에서 발생한 심각한 빌드 오류와 데이터 연동 문제를 해결하여 성공적으로 서비스를 배포했습니다.

### 1. Vercel 빌드 오류 해결 (PostCSS Native Binding 이슈)

**문제 현상:**
- Vercel(Linux 환경)에서 빌드 중 `Failed to load PostCSS config` 및 `Cannot find native binding` 에러 발생.
- `lightningcss` 또는 `rollup`의 리눅스용 바이너리를 찾지 못해 빌드가 중단되는 현상.

**원인 분석:**
- **Tailwind CSS v4 아키텍처 변화**: Tailwind v4는 내부적으로 `lightningcss`를 사용하는데, 기존 PostCSS 방식(`postcss.config.cjs`)을 사용할 경우 배포 환경(Linux)과 개발 환경(Windows) 간의 네이티브 모듈 충돌이 잦음.
- **의존성 정보 누락**: `package-lock.json` 생성 시 개발 환경의 바이너리 정보만 포함되어 Vercel 리눅스 환경에서 필요한 모듈을 내려받지 못함.
- **불필요한 빌드 로직**: 루트 `package.json`에서 빌드 시마다 `node_modules`를 강제로 지우고 다시 설치하는 로직이 Vercel의 캐싱 시스템과 충돌하여 바이너리 설치를 방해함.

**해결 방법 (최종):**
1.  **PostCSS 방식 폐기**: `postcss.config.cjs` 파일을 삭제하고 PostCSS 의존성을 제거.
2.  **Vite 전용 플러그인 도입**: Tailwind CSS v4 전용 Vite 플러그인(`@tailwindcss/vite`)을 도입하여 Vite가 직접 CSS를 처리하도록 변경. 이 방식은 PostCSS 로딩 단계를 생략하므로 네이티브 바인딩 오류를 근본적으로 차단함.
3.  **멀티 플랫폼 아키텍처 설정**: `.npmrc`에 `supportedArchitectures.os=linux,win32`를 명시하여 `package-lock.json`에 두 환경의 정보를 모두 담음.
4.  **빌드 스크립트 단순화**: 루트 `package.json`의 빌드 명령어를 `cd client && npm run build`로 단순화하여 Vercel 표준 설치 프로세스를 따르도록 수정.

### 2. 배포 후 데이터 표시 오류 해결

**문제 현상:**
- 배포 성공 후에도 화면에 주식 데이터가 나타나지 않는 현상 발생.

**원인 및 해결:**
- **환경 변수 주입**: Vercel 프로젝트 설정에 KIS API 및 Supabase 관련 환경 변수(`VITE_...`)가 정확히 입력되었는지 확인.
- **백엔드/프론트엔드 연결**: Vercel 서버리스 함수(`api/*.js`)와 프론트엔드(`client/dist`) 간의 라우팅 설정(`vercel.json`)을 정밀하게 조정하여 `/api` 요청이 올바르게 백엔드 코드로 전달되도록 조치.
- **Node.js 버전 일치**: 루트와 클라이언트의 Node.js 버전을 22.x로 상향 조정하여 최신 라이브러리 호환성 확보.

### 3. 향후 유지보수 가이드
- **Tailwind 설정 변경 시**: `postcss.config.cjs`를 다시 만들지 마십시오. 모든 설정은 `vite.config.ts`와 `tailwind.config.js` 또는 CSS 내의 `@theme` 지시어를 통해 관리합니다.
- **배포 오류 발생 시**: `client/package-lock.json`을 삭제하고 로컬에서 `npm install`을 다시 실행하여 멀티 플랫폼 메타데이터가 포함된 lock 파일을 새로 생성한 뒤 푸시하십시오.

---

## 📝 2026년 2월 9일 업데이트: 홈페이지 성능 최적화 및 테마/종목 정보 표시 리팩토링

오늘은 StockMate 홈페이지의 심각한 성능 문제와 관련된 대규모 리팩토링 및 디버깅 작업을 수행했습니다. 목표는 과도한 API 호출과 클라이언트 측 과부하를 해결하여, 사용자에게 빠르고 안정적인 실시간 주식 정보를 제공하는 것이었습니다.
...
2.  **나머지 카테고리 구현**: 티커 및 테마를 제외한 `급상승`, `급하락`, `거래량`, `거래대금` 등의 시장 순위 카테리에 실제 데이터를 연결하는 작업을 진행합니다. 이를 위해 백엔드에 새로운 API 엔드포인트를 구현하고 프론트엔드를 업데이트할 예정입니다.
...
브라우저와 서버가 기억하고 있던 "잘못된 길(구 버전 cache)"을 강제로 삭제하고 새 길을 찾게 했습니다.
